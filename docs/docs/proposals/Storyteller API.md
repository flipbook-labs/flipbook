...
This function will throw if the return value of `storybookModule` does not conform to [[Storyteller API]], or if the source has a syntax error that `require` would normally fail for.
...
This function will throw if the return value of `storyModule` does not conform to [[Storyteller API]], or if the source has a syntax error that `require` would normally fail for.
...
Each renderer is given its own file so that it’s easy to add on new UI libraries in the future. See [[Story Renderer Spec]]  for more details.
