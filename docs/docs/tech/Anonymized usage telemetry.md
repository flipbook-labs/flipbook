...
aliases: [Metrics to Track]
linter-yaml-title-alias: Metrics to Track
---
...
4. Roblox Studio exited in such a way that the `Unloading` event did not fire
5. The user disabled http requests for Flipbook during the session
...
6. A unique ID is generated when Flipbook is first run
7. This ID will be stored in the user’s plugin settings
8. Any telemetry events will use this ID to maintain continuity
...
9. Setup [`apis.flipbooklabs.com`](http://apis.flipbooklabs.com/) to host backend
10. Use Cloudflare tunnels to manage the DNS
11. Maybe create a new DigitalOcean droplet (or App?) for the service
