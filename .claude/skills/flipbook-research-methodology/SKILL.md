...
**Evidence:** Changed `scripts/lib/run.luau` line 20 from `stdio = "inherit"` to `stdio = nil` (let Lute default). Worked locally. Merged.
...
- Examples:
  - **Path-length mitigation (PR #523, commit 7a6c69b5, 2026-03-12):** Bundle Wally + Loom packages as `.rbxm` files to defeat Windows MAX_PATH. Mechanism: "Rotriever is not picky about content root format." Result: Permanent fix, rbxm bundling in `compileAsync.luau` lines 99–110 still active on current main. Rationale embedded in code comment: "alleviate path length limit issues."
...
3. **Document:** If retiring, add a one-line entry to `flipbook-failure-archaeology` SKILL (also in `.claude/skills/flipbook-failure-archaeology/SKILL.md`) explaining the verdict and when, so future maintainers know: "attempted X in branch Y, did not proceed because Z."
...
**Example: Charm.flags.frozen workaround (src/PluginStarterScript.plugin.luau lines 16–31)**
...
- **BUILD_HASH brittleness:** Verify `scripts/lib/compile.luau` still has the explicit `stdio = "default"` and assertion on lines ~123–125. If Lute stabilizes, this may become unnecessary scaffolding.
- **Path-length mitigation:** Verify rbxm bundling still active in `workspace/flipbook-core/src/build-system/compileAsync.luau` lines ~99–110. Test Windows CI: no path-length failures in last 6 months?
