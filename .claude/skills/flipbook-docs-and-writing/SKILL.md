...
**Location:** `.claude/skills/` and `.agents/skills/` — canonical for agent-facing procedures, maintainer playbooks, and debugging runbooks.

**Audience:** Sonnet-class model, zero-context mid-level engineer.

**Each skill has:** frontmatter (`name`, `description`), when-not-to-use section, provenance/verification commands, date-stamped volatile facts.
...
Create a new SKILL.md under `.claude/skills/<skill-name>/` with this skeleton:
