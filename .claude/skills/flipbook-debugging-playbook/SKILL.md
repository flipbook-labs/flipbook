...
| Watch is not running | In a second terminal, run `lute run build plugin --channel dev --watch` while editing `workspace/flipbook-core/src/StoryView.luau` (add a comment). Do you see "Compiling..." output? | No output, file saved but build doesn't trigger | Check watch roots in `.lute/build.luau` line 189–195 |
...
| Plugin reload skipped | Look at `.lute/build.luau` around line 137: does your command include `--skip-reload`? | `--skip-reload` is in your alias/script | Remove the flag |
...
| Darklua using wrong sourcemap path | Check `.darklua.json` line 10: does it say `"rojo_sourcemap": "./sourcemap-darklua.json"`? | Path is different (e.g., `sourcemap.json`) | Verify you're on current main; the build system may have changed on your branch |
| SOURCE_PATH is absolute instead of relative | Look at `.lute/lib/build-system/compileAsync.luau` line 39; it converts `SOURCE_PATH` to relative: `path.relative(project.REPO_PATH, project.SOURCE_PATH)`. Run the build with full verbosity: `lute run build plugin --channel dev 2>&1 | grep "darklua"` and check the path argument. | Absolute path starting with `/Users/` passed to darklua | This is a path normalization bug in the build script; escalate (unlikely on main) |
...
**Story:** PR #509 migrated from Signals to Charm state management. Storyteller internally mutates Charm-wrapped state, violating Charm's immutability guarantee. Workaround added in `src/PluginStarterScript.plugin.luau` line 31: `Charm.flags.frozen = false` (comment: "evil state bug will lurk in the shadows"). Upstream issue: `flipbook-labs/storyteller#100` (unresolved).
...
| Hypothesis | Experiment | Expected if TRUE | Next step |
|---|---|---|---|
| Workaround disabled | Check line 31 of `src/PluginStarterScript.plugin.luau`; is `Charm.flags.frozen = false` present? | Line is gone or commented out | Add it back immediately; this is a load-bearing line |
...
| Story file not under watch roots | Check `.lute/build.luau` lines 189–195 (excludedFilePatterns). Does your story file match an exclusion? | Your `.story.luau` file is in `code-samples/`, `example/`, or `template/` (prod-pruned dirs) | These dirs are in watch roots but might not recompile in prod channel. Use `--channel dev` |
...
| Store not subscribed per-control | Check `StoryControlRow.luau` line ~30. Does it call `useSignalState(store.getControlValue(controlName))`? | No, it's subscribing to the entire schema | Verify you're on main (or after #576); this is the fix. Re-merge if needed. |
| StoryControlsContext not providing store | Check `StoryView.luau`: does it wrap the story in `StoryControlsContext.Provider`? | No provider, or provider wrapping is wrong level | Add/fix the provider to wrap `<StoryControls />`; see `StoryControls.story.luau` line ~15 for example |
...
| Test place invalid | Check `.lute/test.luau` lines 70–72: what are ROBLOX_UNIT_TESTING_* values? | Hardcoded values don't match .env.template | Sync: universe 6599100156, place 123506190725771 |
...
| BUILD_HASH extraction failed | Look at `.lute/build.luau` line 90: `git rev-parse --short HEAD`. Does git work? | Git not installed or not in PATH | Install git or add to PATH |
| Lute stdio not capturing output | This is the root cause of PR #426/#444. Lute `process.run` behavior changed between versions. Check `.lute/build.luau` line 90–94 for how commitHash is extracted. | No assertion error raised (line 93) but hash is empty | This is a Lute version bug. Check `rokit.toml`: is lute pinned to 1.0.0? If earlier version, update it. |
...
**Fix:** Ensure `.env` exists and has BASE_URL set. Re-run build with `--clean`. Verify `lute run build plugin --channel dev --clean` shows "BUILD_HASH = abc1234" in output (the build script logs this at line 109+).
...
**Story:** PR #593 ("Try and stop Flipbook from popping up uninvited") fixed Studio layout reset triggering auto-launch. Root cause: `DockWidgetPluginGuiInfo.new(..., initEnabled=true, ...)` caused widget to re-enable on layout resets. Fix: changed to `initEnabled=false` and manually controlled open state via toolbar button.
...
**First check:**
```bash
# Verify initEnabled is false
grep "DockWidgetPluginGuiInfo" src/PluginStarterScript.plugin.luau
```

Should show: `Enum.InitialDockState.Float, false, false, 800, 600` (second `false` is initEnabled).

**Discriminating experiments:**

| Hypothesis | Experiment | Expected if TRUE | Next step |
|---|---|---|---|
| initEnabled is true | Check line 35 of `src/PluginStarterScript.plugin.luau` | Second bool is `true` | Change to `false` |
...
**Fix:** Verify `initEnabled=false` in line 35 of `src/PluginStarterScript.plugin.luau`. Re-build: `lute run build plugin --channel dev --clean`. Delete Studio cache if needed.
