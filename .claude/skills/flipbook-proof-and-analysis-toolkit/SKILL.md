...
4. **The fix that landed — an explicit guard in the build script.** This is exactly what main has today (verified 2026-07-01, `.lute/build.luau:100`):
   ```luau
...
2. **Know where each value comes from before build:** BASE_URL, LOG_LEVEL, ENABLE_OUTPUT_LOGGING come from `.env` (copied from `.env.template`); BUILD_VERSION/BUILD_CHANNEL/BUILD_HASH/BUILD_TARGET are computed by `.lute/build.luau` (e.g. `BUILD_HASH = getCommitHash()`, line ~115); JEST_TEST_PATH_PATTERN is set by `.lute/test.luau --filter`.
...
2. **Check build script:** the build script (then `scripts/build.luau`; moved to `.lute/build.luau` in PR #521) computes BUILD_HASH by shelling out to git — today via `getCommitHash()` feeding the Darklua env at `.lute/build.luau:115`. Illustrative shape of the failing pattern:
   ```luau
...
### Worked Example: TreeView/createTreeNodeStore.luau:76 — `:: any` Cast

**Location:** `workspace/flipbook-core/src/TreeView/createTreeNodeStore.luau:76`

**Context:** This file manages the tree view's node state using Charm signals. The comment notes a `:: any` cast.

**What the cast does:** At line 76, a computed signal is cast to `any`:
```luau
local nodeStore = Charm.computed(...) :: any
```
...
**What makes this a proof:** By reading the dependency source, you've proven the contract violation. You didn't guess or test-and-hope; you read the line that breaks it (line 68, `return nil`). That's evidence strong enough to file an upstream issue or patch the dependency.
...
- PR #426 & PR #444 (BUILD_HASH) — grep `BUILD_HASH` in `.lute/build.luau` (line ~115 as of 2026-07-01), check `process.run` stdio behavior in Lute docs
...
- UILabs Object migration — read `Packages/_Index/flipbook-labs_storyteller@*/storyteller/dist/controls/migrations/ui-labs-*/migrateUILabsControl.luau` (the `control.Type == "Object"` branch returning nil; line ~67 in installed 1.11.0 as of 2026-07-01)
