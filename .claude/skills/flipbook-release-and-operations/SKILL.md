...
**Symptom:** Rotriever consumers on Windows MAX_PATH (260 chars) hit nesting limit when unpacking FlipbookCore; CI artifact paths exceeded 260 chars.

**Root cause:** Combining Wally + Loom package managers created nested directory structures; double nesting pushed total over limit.

**Solution (PR #523):** Bundle `Packages/` and `RobloxPackages/` dirs into .rbxms before packaging for Rotriever (in `.lute/lib/build-system/compileAsync.luau` lines 99–110). Reduces path depth.
...
**Lesson:** Windows path length is a real constraint; solutions that compact artifact structure (bundling) are better than detection-only approaches.
